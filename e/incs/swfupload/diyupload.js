...
function challs_flash_onCompleteData(a){ //获取服务器反馈信息事件
        uploadover(a);
        return;
       //document.getElementById('show').innerHTML+='<div style="color:#ff0000">'+a+'</div>';
}

//自定义函数over
function uploadover(a)
 {
   var Result=Request("result",a);
   var CS=Request("cs",a);
   var Url=Request("url",a);
   var thumbnail=Request("thumbnail",a);
   var size=Request("size",a);
   switch(Result)
   {
   case "success":
    document.getElementById('show').innerHTML+="<div style='color:green'>文件上传完毕!</div>";
    parent.GetUpload(Url,thumbnail,size,objid); //必须在父页增加GetUpload函数
   break;
...
function challs_flash_onStart(a){ //开始一个新的文件上传时事件,并传入一个Object类型变量，包括刚上传文件的大小，名称，类型
	...
	var creationDate = a.fileCreationDate //获取文件创建时间
	var modificationDate = a.fileModificationDate //获取文件最后修改时间
	document.getElementById('show').innerHTML+='<font style="color:green">'+name+'开始上传！</font><br>';
...
function challs_flash_onCompleteAll(a){ //上传文件列表全部上传完毕事件,参数 a 数值类型，返回上传失败的数量
	//document.getElementById('show').innerHTML='<font style="color:green">文件上传完毕！</font>';
        if(a!="0")
         {
...
function challs_flash_onSelectFile(a){ //用户选择文件完毕触发事件，参数 a 数值类型，返回等待上传文件数量
	document.getElementById('show').innerHTML='<font style="color:green">'+a+'个文件等待上传完毕！</font><br>'; 
}
