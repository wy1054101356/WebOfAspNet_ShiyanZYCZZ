...
<table border=0 cellpadding=2 cellspacing=0 width=100%  style="table-layout:fixed;" align=center>
<tr>
<td align="right"><a href="javascript:IssueLog()">【我的签发记录】</a></td>
</tr>
</table>
      ...
      <asp:Repeater id="P1" runat="server" OnItemDataBound="Data_Bound">         
             <ItemTemplate>
                 <tr>
                  ...
                  <td align=center  class="memlist_item" style="color:#ff0000"><%#DataBinder.Eval(Container.DataItem,"work_state")%></td>
...
<asp:Label id="Lb_Edit" runat="server"><a href="javascript:Edit('<%#DataBinder.Eval(Container.DataItem,"site_id")%>','<%#Get_Tid(DataBinder.Eval(Container.DataItem,"thetable").ToString())%>','<%#DataBinder.Eval(Container.DataItem,"detail_id")%>','<%#DataBinder.Eval(Container.DataItem,"work_node")%>')">编辑</a></asp:Label>
</td>
                </tr>
             </ItemTemplate>
          </asp:Repeater>
...
<script type="text/javascript">
 ...
 function IssueLog()
  {
   var Url="/e/member/issuelog.aspx?from=member";
   IDialog("签发记录",Url,900,"90%");
  } 

 function Edit(Site_Id,Tid,Id,worknode)
  {
   location.href="/e/member/index.aspx?s="+Sid+"&type=mem_datadtl&tid="+Tid+"&siteid="+Site_Id+"&id="+Id+"&worknode="+worknode;
  }
